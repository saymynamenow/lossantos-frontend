...
const PageBrowser: React.FC = () => {
  ...
  useEffect(() => {
    const loadPages = async () => {
      try {
        ...
        // Load all pages
        const pagesResponse = await apiService.page.getAllPages(1, 50);
        const current = await apiService.user.getCurrentUser();
        setCurrentUser(current);
        const allPages = pagesResponse.pages || [];
        setPages(allPages);

        // Load user's followed pages to determine following states
        try {
          const followedPagesResponse =
            await apiService.page.getUserFollowedPages();
          const followedPageIds = new Set(
            (followedPagesResponse.followedPages || []).map(
              (item: any) => item.page.id
            )
          );

          const initialFollowingStates: { [key: string]: boolean } = {};
          allPages.forEach((page: Page) => {
            initialFollowingStates[page.id] = followedPageIds.has(page.id);
          });
          setFollowingStates(initialFollowingStates);
        } catch (followError) {
          console.error("Failed to load followed pages:", followError);
          // Fallback: initialize all as not following
          const initialFollowingStates: { [key: string]: boolean } = {};
          allPages.forEach((page: Page) => {
            initialFollowingStates[page.id] = false;
          });
          setFollowingStates(initialFollowingStates);
        }
      } catch (error) {
  ...
  const handleFollow = async (pageId: string) => {
    console.log("Following page:", pageId);
    try {
      await apiService.page.followPage(pageId);
      setFollowingStates((prev) => {
        const newState = { ...prev, [pageId]: true };
        console.log("Updated following states:", newState);
        return newState;
      });
      setPages((prev) =>
        prev.map((page) =>
            ...
            <div className="flex flex-col md:flex-row gap-4 bg-white p-6 rounded-xl shadow-md">
              ...
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full text-black pl-10 pr-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
              </div>
              ...
              <select
                ...
                className="px-4 text-black py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white"
              >
